...
class Principal extends Controller
{
    ...
    ## ENVIO DE CAOOREO DE RECUPERACION
    public function enviarCorreo($correo)
    {
        ...
        if (!empty($consulta)) {
            ...
            try {
                ...
                $mail->send();
                $res = array('tipo' => 'success', 'mensaje' => 'EL CORREO FUE ENVIADO CON UN TOKEN DE SEGURIDAD');
            } catch (Exception $e) {
  ...
  //CODIGO PARA CAMBIAR CONTRASEÑA
    public function cambiarPass()
    {
        ...
        if (empty($nueva) || empty($confirmar) || empty($confirmar)) {
            ...
            if ($nueva != $confirmar) {
                $res = array('tipo' => 'warning', 'mensaje' => 'LA CONTRASEÑAS NO COUNCIDEN, VUELVA A INTENTAR');
            } else {
                ...
                if ($token == $result['token']) {
                    ...
                    if ($data == 1) {
                        $res = array('tipo' => 'success', 'mensaje' => 'SE LOGRO CAMBIAR LA CONTRASEÑA');
                    } else {
                        $res = array('tipo' => 'error', 'mensaje' => 'ERROR AL CAMBIAR LA CONTRASEÑA');
                    }
                }
            }
        }
